...
export class RSSFeed {
	...
	init = async (rssUrl: string) => {
		...
		const rawXML = await fetch(rssUrl).then((res) => res.text());
    ...
    this.publisher.rssUrl = rssUrl;
		...
		const rawHTML = this.publisher.url
			? await fetch(`https://corsproxy.io/?url=${this.publisher.url}`).then((res) => res.text())
			: null;
		const domHTML = rawHTML ? new DOMParser().parseFromString(rawHTML, 'text/xml') : null;
...
export class RSSItem {
	...
	constructor(node: Element, publisher: RSSFeed['publisher']) {
		...
		if (content && content.length > MIN_CONTENT_LENGTH) this.content = processHTMLString(content);
		...
		const pubDate = getDate(node.getElementsByTagName('pubDate')?.[0]?.textContent);
		if (pubDate) this.publishedAt = pubDate;

		const updated = getDate(node.getElementsByTagName('updated')?.[0]?.textContent);
		if (updated) this.publishedAt = updated;

		const lastBuildDate = getDate(node.getElementsByTagName('lastBuildDate')?.[0]?.textContent);
